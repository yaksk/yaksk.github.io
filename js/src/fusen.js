(function(global){
	...
	FusenBuilder.prototype.createFusen = function(id, text, color, name) {
        var fusen = new Fusen(this.canvas, this.ds, id, text, color, name);
        this.fusens[id] = fusen;
		return fusen;

	}

	function Fusen(canvas, ds, _id, text, color, name) {
		var self = this;
		this.id = _id;
		this.ds = ds;
        this.pos = {x : 0, y : 0};
        ...
        if (fusen_util.getDevice() == "mobile") {
            this.canvas.append('<div id="'+this.id+'" class="p-husen theme-husen--'+color+'"><div class="husen-detail"><span class="husen-text">'+fusen_util.htmlEscape(text)+'</span><span class="husen-name">From:'+fusen_util.htmlEscape(name)+'</span></div></div>');
        } else {
            this.canvas.append('<div id="'+this.id+'" class="p-husen theme-husen--'+color+'"><div class="husen-detail"><span class="husen-text">'+fusen_util.htmlEscape(text)+'</span><span class="husen-name">From:'+fusen_util.htmlEscape(name)+'</span></div><div class="p-husen__cross">×</div></div>');
        }
